...
> 이 엔드포인트들은 외부에서 직접 호출하지 않습니다. admin-service의 Feign Client
> (`UserAdminClient` / `AiAdminClient` / `StockAdminClient`)가
> `X-Gateway-Secret` 헤더를 포함해 호출하는 내부 전용 API입니다.

**user-service** (UserAdminClient)
...
**ai-service** (AiAdminClient)

| 기능       | URL                                 | Method | 비고                             |
|----------|-------------------------------------|--------|--------------------------------|
| 기업 뉴스 수집 | `/api/v1/admin/ai/news/fetch`       | POST   | Naver News API 기업별 뉴스 수집       |
| 시장 뉴스 수집 | `/api/v1/admin/ai/news/market/fetch` | POST   | 거시 시장 뉴스 수집                    |
| 수동 뉴스 등록 | `/api/v1/admin/ai/news/ticker`      | POST   | Body: NewsCreateRequest (ticker, title, companyName, content, source, url, publishedAt) |

**stock-service** (StockAdminClient)

| 기능         | URL                                    | Method | 비고                      |
|------------|----------------------------------------|--------|-------------------------|
| 한투 종목 업데이트 | `/api/v1/admin/stocks/download/stocks` | POST   | 한국투자증권 API 기반 종목 데이터 갱신 |
